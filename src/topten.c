...
*/
    ...
    case KILLED_BY:
        ...
        l = strlen(buf);
        buf += l, siz -= l;
#else /*JP:既に対象をコピーしているので単に追加*/
        (void) strncat(buf, killed_by_prefix[how], siz - 1);
#endif
        break;
#if 1 /*JP*/
      case KILLED_SUFFIX:
        (void) strncat(buf, "に殺された", siz - 1);
#endif
    }
#if 0 /*JP*//*既にコピー済み*//*JP:TODO:サニタイズが必要*/
    /* Copy kname into buf[].
...
#else
        Fprintf(rfile, "%cwhile=%s", XLOG_SEP,
                multi_reason ? multi_reason : "無力な間に");
#endif
...
*/
            Sprintf(eos(linebuf), "%s", dungeons[t1->deathdnum].dname);
