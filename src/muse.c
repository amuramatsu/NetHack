    ...
    if (obj->oclass == WAND_CLASS && obj->cursed
        ...
        if (vis) {
            ...
            pline("%s zaps %s, which suddenly explodes!", Monnam(mon),
                  an(xname(obj)));
#else
            pline("%sが%sをふりかざすと，それは突然爆発した！", Monnam(mon),
                  an(xname(obj)));
#endif
        } else {
...
boolean self;
{
    if (!canseemon(mtmp)) {
        ...
        pline("%s zaps %sself with %s!", Monnam(mtmp), mhim(mtmp),
              doname(otmp));
#else
        pline("%sは自分自身に%sをふりかざした！", Monnam(mtmp),
              doname(otmp));
#endif
    } else {
/*JP
...
#else
        pline("%sは%sに向けて%sを吹いた！", Monnam(mtmp),
              self ? mon_nam_too(mtmp, mtmp) : (char *) "あなた",
              xname(otmp));
#endif
...
*/
                pline("%sは一瞬方向感覚を失った．", Monnam(mtmp));
            return 2;
        }
        if (oseen && how)
            makeknown(how);
...
*/
            pline("%sははしごをのぼって逃げた！", Monnam(mtmp));
...
*/
            pline("%sははしごを降りて逃げた！", Monnam(mtmp));
...
*/
            pline("%sはとても気分がよくなったようだ．", Monnam(mtmp));
        if (oseen)
...
*/
            pline("%sは完全に回復したようだ．", Monnam(mtmp));
        if (oseen)
            makeknown(otmp->otyp);
        m_useup(mtmp, otmp);
        return 2;
...
*/
            pline("%sは%sを強く投げつけた！", Monnam(mtmp), singular(otmp, doname));
        }
...
*/
            pline("%sは経験を積んだように見える．", Monnam(mtmp));
        if (oseen)
...
*/
            ...
            if (vismon)
                ...
                pline("%s flicks a bullwhip towards your %s!", Monnam(mtmp),
                      hand);
#else
                pline("%sはあなたの%sに向かって鞭を打った！", Monnam(mtmp),
                      hand);
#endif
...
*/
            ...
            pline("%s wraps around %s you're wielding!", The_whip,
                  the_weapon);
#else
            pline("%sはあなたの装備している%sにからみついた！", The_whip,
                  the_weapon);
#endif
            if (welded(obj)) {
#if 0 /*JP:T*/
            ...
            case 1: /* onto floor beneath mon */
                ...
                pline("%s yanks %s from your %s!", Monnam(mtmp), the_weapon,
                      hand);
#else
                pline("%sは%sをあなたの%sからぐいとひっぱった！", Monnam(mtmp), the_weapon,
                      hand);
#endif
            ...
            case 2: /* onto floor beneath you */
                ...
                pline("%s yanks %s to the %s!", Monnam(mtmp), the_weapon,
                      surface(u.ux, u.uy));
#else
                pline("%sは%sを%sに引き落した！", Monnam(mtmp), the_weapon,
                      surface(u.ux, u.uy));
#endif
                dropy(obj);
                break;
...
*/
                pline("%sは%sを奪った！", Monnam(mtmp), the_weapon);
...
*/
            pline("%sはまた見えるようになった．", Monnam(mon));
    }
}
...
*/
            pline("%sは胃酸の調子がとても悪い．", Monnam(mon));
        if (DEADMONSTER(mon)) {
/*JP
...
*/
            pline("%sはより丈夫になったようだ．", Monnam(mon));
    }
