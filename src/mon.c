...
#else /*hliquidは不自然になるのでとりあえず使わない*/
...
*/
            pline("残念ながら%sは虐殺されている．．．", mon_nam(mtmp));
        mtmp->mhp = 0;
    }
}
    ...
    if (is_vampshifter(mtmp)) {
        ...
        if (mndx >= LOW_PM && mndx != monsndx(mtmp->data)
            ...
            Sprintf(buf, "%s%sは突然%s，%%sとして蘇った！",
                    spec_death ? "" : "死んだように思われた",
                    x_monnam(mtmp, ARTICLE_NONE, (char *) 0,
                             SUPPRESS_SADDLE | SUPPRESS_HALLUCINATION
                                 | SUPPRESS_INVISIBLE | SUPPRESS_IT,
                             FALSE),
                    spec_death ? "再構成され" : "変化し");
#endif
            mtmp->mcanmove = 1;
            mtmp->mfrozen = 0;
            if (mtmp->mhpmax <= 0)
...
*/
                    pline("%sの中で爆発が起きた！", body_part(STOMACH));
#if 0 /*JP*/
                    Sprintf(killer.name, "%s explosion",
                            s_suffix(mdat->mname));
#else
                    Sprintf(killer.name, "%sの爆発で", mdat->mname);
#endif
...
#else
            You("新しくできた%sから%s．",
                xname(otmp), jumpedthrough(youmonst.data, "飛び出た"));
#endif
    }
}
...
#else
            } else if (!flooreffects(otmp, x, y, nomsg ? "" : "落ちる")) {
#endif
...
*/
                pline("%sの死体は埋まってしまった．", Monnam(mtmp));
            }
        }
    }
    if (wasinside)
...
*/
                pline("なんと%sになってしまった．", mtmp->data->mname);
        } else {
            if (canseemon(mtmp))
/*JP
...
#else
        if (!strcmpi(newname, "何者か")) { /* can't see or sense it now */
#endif
#if 0 /*JP*/
