...
*/
    { "altkeyhandler", "ALTキーハンドラ", 20, SET_IN_GAME },
...
*/
      "パレット (00c/880/-fffはそれぞれ青/黄/反転白を示す)", 15, SET_IN_GAME
#else
/*JP
      "palette (adjust an RGB color in palette (color-R-G-B)", 15, SET_IN_FILE
*/
      "パレット (パレットのRGB色を調整する (色-R-G-B)", 15, SET_IN_FILE
#endif
    },
#if defined(MAC)
/*JP
        ...
        if (op == empty_optstr) {
            ...
            if (flags.menu_style == MENU_TRADITIONAL
                ...
                Sprintf(qbuf, "New %s: [%s am] (%s)", fullname, ocl,
                        *tbuf ? tbuf : "all");
#else
                Sprintf(qbuf, "新しい%s: [%s am] (%s)", fullname, ocl,
                        *tbuf ? tbuf : "all");
#endif
                abuf[0] = '\0';
                getlin(qbuf, abuf);
...
*/
        opt_idx = handle_add_list_remove("自動拾い例外", numapes);
        if (opt_idx == 3) { /* done */
            return TRUE;
        } else if (opt_idx == 0) { /* add new */
...
char *buf;
{
    ...
    if (!strcmp(optname, "align_message")
        ...
        Sprintf(buf, "%s",
                (which == ALIGN_TOP) ? "上側"
                : (which == ALIGN_LEFT) ? "左側"
                  : (which == ALIGN_BOTTOM) ? "下側"
                    : (which == ALIGN_RIGHT) ? "右側"
                      : defopt);
#endif
